...
library(ggplot2)
boxplot(A5$Okuma_yazma_bilen_fakat_bir_okul_bitirmeyen,horizontal = TRUE)
ggplot(A5, aes(x = "??ller", y = Okuma_yazma_bilen_fakat_bir_okul_bitirmeyen)) +
  geom_boxplot(fill = "#FF69B4",    
               color = "#000080") + 
  labs(title = "2023 Y??l??nda Turkiye Genelinde Okuma Yazma Bilen Ama Okul Bitirmemi?? Ki??i Say??s??n??n Da????l??m??",
       x = "",
       y = "Ki??i Say??s??") +
  theme_minimal()
